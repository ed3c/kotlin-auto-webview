...
| #53 | Durable and multi-node replay state | `DurableMcpHttpReplayGuard` | [0024](architecture/ADR-0024-durable-replay-state.md) | durability tested; multi-node coordination **was falsified and fixed** under #57 — see the ADR's refutation |
| ~~#39~~ | Established-session recovery probe | `DeepSeekHarnessEstablishedSessionRecoveryE2ETest` | [0016](architecture/ADR-0016-deepseek-established-session-recovery-probe.md) | **Answered: `FAIL_TRANSPORT_CLOSE_NOT_OBSERVED`** at the pinned upstream subject (run 31886125611). An admitted outcome, not a defect in this repository; the oracle is preserved unweakened and excluded from the recurring run |
| #49 | Generated Cordis patch parsing, HMR, tool-list replacement | `DeepSeekHarnessCordisBinding` | [0025](architecture/ADR-0025-deepseek-cordis-patch-as-configuration.md) | patch parse and the secret boundary **PASS** against the pinned loader; HMR and tool-list replacement are `NOT_EXERCISABLE_IN_THIS_LANE` — mounting needs the package's built entry, which the pinned `--ignore-scripts` install cannot produce |
...
| ~~#63~~ | A tracked source silently becoming binary to git | this landing | **Closed.** `scripts/ci/check-text-sources.py` fails on an undeclared NUL and names the byte offset; run against `92e89a1` it reports the original defect at offset 1108 |
| ~~#57~~ | `multi_node_replay_coordination` | [0024](architecture/ADR-0024-durable-replay-state.md) | **Closed, and it found the claim false.** The first cross-process test showed four contenders all admitting one digest: `Files.readAllLines` inside the locked block opened and closed a second descriptor, and POSIX releases every lock a process holds on a file when any of its descriptors closes. Fixed by reading through the locked descriptor; proven by `DurableMcpHttpReplayGuardMultiProcessTest` |
...
| ~~#59~~ | `jwks_endpoint_fetching_and_caching`, revocation | [0026](architecture/ADR-0026-mcp-jwks-key-retrieval.md) | **Closed.** `McpJwksKeySource` retrieves over HTTPS with a bounded cache, a refresh rate limit that applies to every reason (so an attacker-chosen `kid` cannot amplify requests), fail-closed expiry, and wholesale replacement so retirement propagates |
| ~~#60~~ | `os_keychain_or_hsm_custody` | [0027](architecture/ADR-0027-host-key-store-custody.md) | **Closed for macOS.** OS-backed custody with typed absence reasons, revocation that reaches storage, and an epoch that survives restart. Linux Secret Service round-trip `NOT_EXERCISED`; Windows `NOT_IMPLEMENTED` rather than shipped-unverified |
