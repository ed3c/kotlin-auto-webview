...
internal fun googleProjectionSha256(value: String): String =
    googleProjectionSha256(value.encodeToByteArray())

internal fun googleProjectionSha256(bytes: ByteArray): String {
    val bitLength = bytes.size.toLong() * 8L
    val zeroPadding = (56 - ((bytes.size + 1) % 64) + 64) % 64
    val padded = ByteArray(bytes.size + 1 + zeroPadding + 8)
    ...
    while (chunk < padded.size) {
        ...
        for (index in 0 until 64) {
            ...
            val temp1 = h + sum1 + choose + SHA256_CONSTANTS[index] + words[index]
    ...
    return buildString(64) {
        for (word in intArrayOf(h0, h1, h2, h3, h4, h5, h6, h7)) {
